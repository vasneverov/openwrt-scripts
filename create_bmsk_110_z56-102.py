Добавь, да, добавь, конечно, в шесто тридцать первый порт. Если он работает, то да.#!/usr/bin/env python3
"""Create new inbound on bMSK port 110 (POP3) for router z56-102"""
import asyncio
import aiohttp
import ssl
import json
...
PRIVATE_KEY = "QKPZitEi4nYrAUo_1SqK4lagqkg9RCI89ZCtFey8CUI"
PUBLIC_KEY = "Xm39CXeF9Z8baBwE4EyyUgUtC6VIrguyn-aw6Q4lOAM"
SHORT_ID = "b4e8c3d2"
NEW_PORT = 110
SNI = "www.apple.com"

ROUTER_NAME = "z56-102"
CLIENT_EMAIL = f"{ROUTER_NAME}_bMSK_{NEW_PORT}"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(list_url, headers={"Cookie": f"3x-ui={cookie}"}, ssl=ctx) as resp:
            ...
            if result.get("success"):
                ...
                # Check if port 110 already exists
                for ib in inbounds:
                ...
                async with session.post(add_url, json=payload, headers={"Cookie": f"3x-ui={cookie}"}, ssl=ctx) as resp2:
                    ...
                    if result2.get("success"):
                        ...
                        if str(NEW_PORT) in check.stdout:
                            ...
                            print(f"{'='*70}")
                            print(f"\n📋 Server: {PANEL_IP}")
                            print(f"📋 Port: {NEW_PORT} (POP3 - low port)")
