# Инструкции для Claude — прошивка роутеров
...
## Универсальный скилл прошивки

Главный скилл для прошивки любого роутера:
```
skills/flash_router_universal.md
```

При команде "прошиваем роутер" — сразу открывать этот скилл.
Скилл сам:
- Определяет модель
- Вызывает субскилл создания ключей (через SQLite базу, не через API панели)
- Знает relay-серверы, inbound-ы, pbk/sid для каждой модели
- После ребута — самопроверка что всё поднялось
...
3. **Два IP-адреса в процессе прошивки:**
...
```
На вопросы: y = обновление, y = https-dns-proxy, y = русский язык.
...
1. Открыть `skills/flash_router_universal.md` — единый скилл для любой модели
2. Скилл сам определит модель, relay, inbound, pbk/sid
3. Создание ключей — через SQLite базу (не через API панели)
...
7. После ребута — самопроверка что всё поднялось
8. Итоговый отчёт: Tailscale ✅, Podkop main ✅, Podkop YT ✅
