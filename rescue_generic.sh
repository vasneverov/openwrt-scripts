...
echo "[1/11] Tailscale fw_mode → none"
...
echo "[2/11] init.d/tailscale → DISABLED"
...
echo "[4/11] ulimit + sysctl → увеличение лимитов"
...
echo "[5/11] Podkop exclude_ntp → 1"
...
# ===== 6. podkop-fw4-fix.sh — nftables fix для fw4 =====
echo "[6/11] podkop-fw4-fix.sh → установка"
...
case "$ACTION" in
  update|add)
    # Добавляем правило в mangle_forward для пропуска podkop-трафика
    # Проверяем, есть ли уже — чтобы не дублировать
    for MARK in 0x00100000 0x00010000 0x00020000 0x00040000; do
      if ! nft -a list chain inet fw4 mangle_forward 2>/dev/null | grep -q "meta mark $MARK accept"; then
        nft add rule inet fw4 mangle_forward meta mark $MARK accept comment \"podkop-fw4-fix\" 2>/dev/null || true
      fi
    done
    ;;
  check)
    COUNT=$(nft list chain inet fw4 mangle_forward 2>/dev/null | grep -c 'podkop-fw4-fix' || echo 0)
    echo "$COUNT"
    ;;
  *)
...
echo "  ✓ /root/podkop-fw4-fix.sh создан"

# ===== 7. rc.local — новый формат (без sleep 40, с timeout + fw4-fix) =====
echo "[7/11] rc.local → новый формат"
if [ -f /etc/rc.local ] && grep -q "for i in 1 2 3" /etc/rc.local 2>/dev/null; then
    echo "  ✓ rc.local уже новый (timeout + fw4-fix)"
else
...
EOF
    ...
    echo "  ✓ rc.local создан (timeout + fw4-fix + watchdog)"
fi



# ===== 8. firewall → tailscale0 в LAN зону =====
...
# ===== 9. Три watchdog'а на 2 минуты =====
echo "[9/11] Watchdog'ы (3 шт, каждая 2 мин)..."
...
# ===== 10. Crontab =====
echo "[10/11] Crontab → 3 watchdog'а + обновление списков"
...
# ===== 11. check-ip скрипт диагностики =====
echo "[11/11] check-ip → скрипт диагностики"
