"""Financial report generator v6 — grouped entries + router movement."""
import re
import logging
...
def generate(vault_path: Path, day: date) -> str:
    ...
    if today_m:
        ...
        for line in body.split("\n"):
            ...
            if cat_clean not in today_by_cat:
                today_by_cat[cat_clean] = [0, bank, 0]
            today_by_cat[cat_clean][0] += int(val)
            today_by_cat[cat_clean][2] += 1
    ...
    if today_by_cat:
        ...
        for cat, (amt, bank, count) in sorted(today_by_cat.items()):
            sign = "+" if amt >= 0 else ""
            if count > 1:
                lines.append(f"• {cat}: {sign}{_num(amt)} ₽ ({bank}) ×{count}")
            else:
                lines.append(f"• {cat}: {sign}{_num(amt)} ₽ ({bank})")
        lines.append("")
    ...
    # ── 5. ROUTER MOVEMENT (today) ──
    router_moves = []
    inv_section = re.search(r"### Инвентарь оборудования.*?(?=\n### |\n---|\Z)", memory_text, re.DOTALL)
    if inv_section:
        for line in inv_section.group(0).split("\n"):
            if day.strftime("%d.%m") in line or day.strftime("%d.%m").lstrip("0") in line:
                clean = line.lstrip("- *").strip()
                if clean and "Закупка" not in clean and clean not in router_moves:
                    router_moves.append(clean)

    if router_moves:
        lines.append("🔧 <b>ДВИЖЕНИЕ РОУТЕРОВ:</b>")
        for rm in router_moves:
            lines.append(f"• {rm}")
        lines.append("")
    ...
    lines.append(f"• ФИНАНСЫ {day_display} → {len(today_by_cat)} категорий")
    if router_moves:
        lines.append(f"• Роутеры → {len(router_moves)} изменений")
    if debtors:
        lines.append(f"• Дебиторка → {len(debtors)} актуальных")
    lines.append("")

    lines.append("---")
    lines.append(f"<i>Обработано {sum(v[2] for v in today_by_cat.values())} записей за ~{datetime.now().strftime('%M')} мин</i>")
